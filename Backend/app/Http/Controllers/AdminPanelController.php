...
class AdminPanelController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        if (!$user?->role || $user->role->label !== 'admin') {
            ...
            return back()->withErrors(['email' => 'No tienes permisos para acceder al panel administrativo']);
        }
    ...
    public function store(Request $request, string $resource)
    {
        ...
        $validated = $request->validate($config['rules']['create']);
        $validated = $this->preparePayload($resource, $validated, 'create');

        $model = $config['model'];
        $model::create($validated);

        return redirect()
            ->route('admin.resource.index', ['resource' => $resource])
    ...
    public function edit(string $resource, int $id)
    {
        ...
        return view('admin.resources.form', [
            ...
            'item' => $item,
            'mode' => 'edit',
            'action' => route('admin.resource.update', ['resource' => $resource, 'id' => $id]),
            'method' => 'PUT',
        ]);
    }
    ...
    public function update(Request $request, string $resource, int $id)
    {
        ...
        $validated = $request->validate($config['rules']['update']($id));
        $validated = $this->preparePayload($resource, $validated, 'update', $item);
    ...
    private function resourceConfig(string $resource): array
    {
        $resources = [
            'roles' => [
                ...
                'allowCreate' => true,
                'allowEdit' => true,
                'query' => fn () => Role::orderByDesc('id')->paginate(10),
                'with' => [],
                'columns' => [
                    ['label' => 'ID', 'key' => 'id'],
            ...
            'room_types' => [
                ...
                'allowCreate' => true,
                'allowEdit' => true,
                'query' => fn () => RoomType::orderByDesc('id')->paginate(10),
                'with' => [],
                'columns' => [
                    ['label' => 'ID', 'key' => 'id'],
                    ['label' => 'Nombre', 'key' => 'name'],
            ...
            'rooms' => [
                ...
                'allowCreate' => true,
                'allowEdit' => true,
                'query' => fn () => Room::with('roomType')->orderByDesc('id')->paginate(10),
                'with' => ['roomType'],
                'columns' => [
                    ['label' => 'ID', 'key' => 'id'],
                ...
                'fields' => [
                    ...
                    [
                        ...
                        'type' => 'select',
                        'options' => fn () => RoomType::orderBy('name')->get()->map(fn ($roomType) => [
                            'value' => $roomType->id,
                            'label' => $roomType->name,
                        ])->all(),
                    ],
                ...
                'rules' => [
                    ...
                    'update' => fn (int $id) => [
                        'number' => ['required', 'string', 'max:255', Rule::unique('rooms', 'number')->ignore($id)],
                        'room_type_id' => ['required', 'integer', 'exists:room_types,id'],
                        'price' => ['required', 'numeric', 'min:0'],
                        'status' => ['required', Rule::in(['disponible', 'ocupada', 'mantenimiento'])],
                        'description' => ['required', 'string', 'max:255'],
            ...
            'users' => [
                ...
                'allowCreate' => true,
                'allowEdit' => true,
                'query' => fn () => User::with('role')->orderByDesc('id')->paginate(10),
                'with' => ['role'],
                'columns' => [
                    ['label' => 'ID', 'key' => 'id'],
                    ['label' => 'Nombre', 'key' => 'name'],
                ...
                'fields' => [
                    ...
                    [
                        ...
                        'label' => 'Rol',
                        'type' => 'select',
                        'options' => fn () => Role::orderBy('name')->get()->map(fn ($role) => [
                            'value' => $role->id,
                            'label' => $role->label,
                        ])->all(),
                    ],
            ...
            'payments' => [
                ...
                'allowCreate' => false,
                'allowEdit' => true,
                'query' => fn () => Payment::with(['reservation.user', 'reservation.room'])->orderByDesc('id')->paginate(10),
                ...
                'fields' => [
                    [
                        ...
                        'label' => 'Reserva',
                        'type' => 'select',
                        'options' => fn () => Reservation::with(['user', 'room'])->orderByDesc('id')->get()->map(fn ($reservation) => [
                            'value' => $reservation->id,
                            'label' => 'Reserva #' . $reservation->id . ' - ' . $reservation->user?->name . ' - Habitación ' . $reservation->room?->number,
                        ])->all(),
                    ],
        ...
        $config['rules']['create'] = $config['rules']['create'] ?? [];
        $config['rules']['update'] = $config['rules']['update'] ?? fn () => [];
        $config['fields'] = array_map(function (array $field) {
            if (array_key_exists('options', $field) && $field['options'] instanceof \Closure) {
    ...
    private function preparePayload(string $resource, array $payload, string $mode, $item = null): array
    {
        if ($resource === 'users' && array_key_exists('password', $payload)) {
            if ($mode === 'update' && ($payload['password'] === null || $payload['password'] === '')) {
        ...
        if ($resource === 'payments') {
            $reservation = Reservation::findOrFail($payload['reservation_id']);
